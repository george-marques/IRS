...
const pool = new Pool({ // Cria uma nova pool de conexões para o PostgreSQL.
  host: 'database', // O hostname do banco de dados PostgreSQL.
  database: 'meubanco', // O nome do banco de dados.
  user: 'usuario', // O nome do usuário do banco de dados.
  password: 'senha', // A senha do usuário do banco de dados.
});
...
app.get('/', async (req, res) => { // Define uma rota GET na raiz ("/").
...
app.post('/cliente', async (req, res) => { // Define uma rota POST em "/cliente".
  const { nome, idade } = req.body; // Extrai os campos nome e idade do corpo da solicitação.
  const client = await pool.connect(); // Conecta-se ao banco de dados.
  await client.query('INSERT INTO cliente (nome, idade) VALUES ($1, $2)', [nome, idade]); // Executa uma consulta SQL para inserir um novo registro na tabela cliente.
